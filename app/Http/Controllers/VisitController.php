...
class VisitController extends Controller
{
    ...
    function __construct() {
        $this -> params = "projects.id, title, synopsis, projects.genre_id, projects.user_id, projects.city_id, username, duration, countries.sortname,
           IFNULL(myfollow.follow, 0) as myfollow, IFNULL(recommendations.reason,'') as recommendation, IFNULL(mylove.love, 0) as mylove,
           IFNULL(view.cnt, 0) as views_cnt, IFNULL(follower.cnt, 0) as followers_cnt, IFNULL(comment.cnt, 0) as comments_cnt, IFNULL(lovers.cnt, 0) as lovers_cnt,  
    ...
    protected function choose(){
        return Project::join('users', 'users.id', '=', 'projects.user_id')
            ...
            ->leftJoin(DB::raw("(select count(id) as follow, project_id from project_followers where user_id = '".Auth::id()."' group by project_id) myfollow"), 'projects.id', '=', 'myfollow.project_id')
    ...
    {
        $this->params.= ', genres.name_' . Auth::user()->locale . ' as genre_name, cities.name_' . Auth::user()->locale . ' as city_name';
        if(!is_null($str)){
            $this->params.=','.$str;
        }

        return $projects->selectRaw( $this -> params);
            /* 'projects.id', 'title', 'synopsis', 'genre_id', 'genres.name_' . Auth::user()->locale . ' as genre_name', 'projects.user_id', 'username', 'duration',
            'projects.city_id', DB::raw('cities.name_' . Auth::user()->locale . ' as city_name'), DB::raw('IF(recommendations.project_id IS NULL, 0, 1) as recommended'),
