...
class RelationController extends Controller
{
    ...
    public function mine(){
        ...
        $city = City::select('department_id', 'name_'.Auth::user()->locale)->find(Auth::user()->city_id);
    ...
    public function friends($id)
    {
        return User::join(DB::raw("(select fan_id, updated_at from relations where relations.idol_id = '".$id."' and love = 1) friends"), function ($join) {
            ...
            ->selectRaw("users.id, users.username, users.presentation, sortname, users.city_id, cities.name_".Auth::user()->locale." as city_name,
    ...
    public function fans($id)
    {
        ...
        return User::join(DB::raw("(select fan_id, love, updated_at from relations where relations.idol_id = '".$id."') friends"), function ($join) {
            ...
            ->join(DB::raw("(select id, department_id, name_".Auth::user()->locale." as city_name from cities) city"), function ($join) {
            ...
            ->join(DB::raw("(select id, sortname from countries) country"), function ($join) {
            ...
            ->selectRaw("users.id, users.username, city.id as city_id, users.presentation, country.sortname, city.city_name, friends.love,
            IFNULL(fans.cnt, 0) as fans_cnt, IFNULL(idols.cnt, 0) as idols_cnt,
    ...
    public function idols($id)
    {
        return User::join(DB::raw("(select idol_id, love, updated_at from relations where relations.fan_id = '".$id."') friends"), function ($join) {
            ...
            ->selectRaw("users.id, users.username, users.city_id, users.presentation, sortname, cities.name_".Auth::user()->locale." as city_name, friends.love,
                IFNULL(fans.cnt, 0) as fans_cnt, IFNULL(idols.cnt, 0) as idols_cnt,
                (CASE WHEN users.id = '".Auth::id()."' THEN 'Self' ELSE IFNULL(myfans.relation, myidols.relation) END) as relation")
            ->orderBy('friends.updated_at')
            ->paginate(8);
